...
def print_packet(pktlen, data, timestamp):
  ...
  if data[12:14]=='\x08\x00':
    ...
    packet_data = {}
    packet_data['timestamp'] = '%s.%f' % (time.strftime('%H:%M',
                                           time.localtime(timestamp)),
                             timestamp % 60)
    for key in ['source_address', 'destination_address', 'version',
                'header_len', 'tos', 'total_len', 'id', 'flags',
                'fragment_offset', 'ttl', 'checksum']:
        packet_data[key] = decoded[key]
    packet_data['protocol'] = protocols[decoded['protocol']]
